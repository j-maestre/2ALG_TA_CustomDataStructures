...
int main()
{
	...
	error_type = node_1->setData(NULL, 4);
	...
	error_type = node_1->memCopy(NULL, 4);
	printFunctionResult(node_1, (u8*)"memCopy(node_1, NULL, 4)", error_type);
	error_type = node_1->memCopy((void*)ptr_data_1, 0);
	...
	error_type = node_1->memConcat(NULL, 4);
	...
	node_3 = MemoryNode::Create();
	if (NULL == node_3) {
		printf("\n create returned a null node in node_3");
		return -1;
	}
	error_type = node_3->setData(ptr_data_4, kSizeData4);
	...
	for (int i = 0; i < kIterationsBigData; ++i)
	{
		...
		error_type = node_2->memCopy(node_3->data(), node_3->size());
