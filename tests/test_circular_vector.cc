...
int main() {
	...
	CircularVector* v = NULL;
	v = CircularVector::Create(1);
	if (NULL == v) {
		printf("\n create returned a null node in vector for ops");
		return -1;
	}
	// vectors to work with
	CircularVector* vector_1 = NULL;
	CircularVector* vector_2 = NULL;
	CircularVector* vector_3 = NULL;
	vector_1 = CircularVector::Create(kCapacityVector1);
	if (NULL == vector_1) {
		printf("\n create returned a null node in vector_1\n");
		return -1;
	}
	vector_2 = CircularVector::Create(kCapacityVector2);
	if (NULL == vector_2) {
		printf("\n create returned a null node in vector_2\n");
		return -1;
	}
	vector_3 = CircularVector::Create(kCapacityVector3);
	if (NULL == vector_3) {
	...
	printf("\n\n---------------- NULL BATTERY ----------------\n\n");
	printf("\n\n# Test Create\n");
	vector_2 = CircularVector::Create(0);
	if (NULL != vector_2) {
