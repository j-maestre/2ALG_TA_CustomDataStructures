...
int main() {
	...
	CVector *vector_1 = NULL;
	CVector *vector_2 = NULL;
	CVector *vector_3 = NULL;
	vector_1 = CVECTOR_create(kCapacityVector1);
	if (NULL == vector_1) {
		printf("\n create returned a null node in vector_1\n");
		return -1;
	}
	vector_2 = CVECTOR_create(kCapacityVector2);
	if (NULL == vector_2) {
		printf("\n create returned a null node in vector_2\n");
		return -1;
	}
	vector_3 = CVECTOR_create(kCapacityVector3);
	if (NULL == vector_3) {
	...
	TESTBASE_printFunctionResult(v, (u8 *)"destroy MVector Operations", error_type);
